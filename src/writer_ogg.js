...
OggWriter.prototype.writeHeaders = function () {
  ...
  var out = Buffer.allocUnsafe(head.length + tags.length);
  head.copy(out, 0);
  tags.copy(out, head.length);
  return out;
};
...
OggWriter.prototype.writeEos = function () {
  return this._buildPage(Buffer.alloc(0), FLAG_EOS, this._granule);
};
...
OggWriter.prototype._buildIdPage = function () {
  var body = Buffer.alloc(19);
  body.write('OpusHead', 0, 'ascii');    // 0..7
  body.writeUInt8(1, 8);                 // version
  body.writeUInt8(this._channels, 9);    // channel count
  body.writeUInt16LE(this._preSkip, 10); // pre-skip (samples @ 48 kHz)
  body.writeUInt32LE(this._inputRate, 12);  // original input sample rate
  body.writeInt16LE(0, 16);              // output gain (Q8 dB, 0 = flat)
  body.writeUInt8(0, 18);                // channel mapping family = 0
...
OggWriter.prototype._buildTagsPage = function () {
  var vendor = Buffer.from(this._vendor, 'utf8');
  var body = Buffer.allocUnsafe(8 + 4 + vendor.length + 4);
  body.write('OpusTags', 0, 'ascii');
  body.writeUInt32LE(vendor.length, 8);
  vendor.copy(body, 12);
  body.writeUInt32LE(0, 12 + vendor.length);   // user comment list length
...
OggWriter.prototype._buildPage = function (body, flags, granule) {
  ...
  var page = Buffer.allocUnsafe(headerLen + body.length);

  // Fixed-header portion
  page.write('OggS', 0, 'ascii');                   // 0..3  sync
  page.writeUInt8(0, 4);                            // 4     version
  page.writeUInt8(flags, 5);                        // 5     type flags
  ...
  page.writeUInt32LE(granLo, 6);
  page.writeUInt32LE(granHi, 10);
  page.writeUInt32LE(this._serial, 14);             // 14..17 bitstream serial
  page.writeUInt32LE(this._sequence, 18);           // 18..21 page sequence
  page.writeUInt32LE(0, 22);                        // 22..25 CRC placeholder
  page.writeUInt8(segments.length, 26);             // 26     num segments
  ...
  // Body
  if (body.length) body.copy(page, headerLen);
