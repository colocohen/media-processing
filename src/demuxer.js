...
import { extractParameterSetsAnnexB } from './reader_annexb.js';
...
Demuxer.prototype.start = function (opts) {
  ...
  if (hasVideo) {
    ...
    if (vc === 'h264') args.push('-map', '0:v:0', '-c:v', 'copy', '-bsf:v', 'h264_mp4toannexb,h264_metadata=aud=insert', '-f', 'h264', 'pipe:3');
    else if (vc === 'h265') args.push('-map', '0:v:0', '-c:v', 'copy', '-bsf:v', 'hevc_mp4toannexb,hevc_metadata=aud=insert', '-f', 'hevc', 'pipe:3');
    ...
    if (vContainerDef && vContainerDef.createReader) {
      ...
      vReader.on('video', function (f) {
        ...
        if (f.isKeyframe && !self._videoDescription) {
          var vc = info.video.codec;
          if (vc === 'h264' || vc === 'h265') {
            self._videoDescription = extractParameterSetsAnnexB(f.payload, vc === 'h265');
          } else if (vc === 'av1') {
