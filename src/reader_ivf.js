...
function IVFReader() {
  ...
  this._buf = Buffer.allocUnsafe(128 * 1024);
  this._start = 0;
  this._end = 0;
}
...
IVFReader.prototype.feed = function (chunk) {
  ...
  if (!this._gotHeader) {
    ...
    this._fourcc = b.toString('ascii', s + 8, s + 12);
    this._width = b.readUInt16LE(s + 12);
    this._height = b.readUInt16LE(s + 14);
    this._timebaseDen = b.readUInt32LE(s + 16);
    this._timebaseNum = b.readUInt32LE(s + 20);

    var ivfHeader = Buffer.from(b.subarray(s, s + 32));
    this._start += 32;
  ...
  while (true) {
    ...
    var b2 = this._buf;
    var s2 = this._start;
    var frameSize = b2.readUInt32LE(s2);
    var tsLo = b2.readUInt32LE(s2 + 4);
    var tsHi = b2.readUInt32LE(s2 + 8);
...
function splitOBUs(buf) {
  ...
  while (p < buf.length) {
    ...
    obus.push({
      ...
      data: Buffer.from(buf.subarray(p, end)),
    });
    p = end;
  }
  return obus;
